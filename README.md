# OutlastMM - a fork of the OutlastTogether repository by MeinaWithAI (Meina)

![OMM_1](https://github.com/Heawikk/OutlastMM/blob/main/Prikol/3.jpg)

# Install & Run
1. Download the repository.
2. Extract the folder to any convenient location (make sure there are no Cyrillic characters in the file path).
3. Copy the `OLGame` folder into the game's root directory.
4. In the game root directory, open `OLGame/Config/DefaultMultiplayer.ini` and enter the server's IP and port.
5. In the main folder, open the config.ini file and replace the path with the path to your game's executable file.
6. If you are the host, launch the game via `run_server.bat`. If you are a player, use one of the `run_client_#.bat` files (make sure that no two players launch a batch file with the same number).
7. Load into the same checkpoint.
8. Enjoy!
